...
문서 규칙은 가능한 한 도구로 강제하는 것을 목표로 합니다. 현재 `eslint.config.mjs`는 Next.js 기본 설정(`core-web-vitals`, `typescript`)과 ignore override만 포함하므로, 아래 import 경계 규칙은 아직 적용된 설정이 아니라 적용 후보 예시입니다. 실제로 도입할 때는 `eslint.config.mjs`와 이 문서를 함께 갱신합니다.

- 현재 적용: TypeScript path alias(`@/`)는 `tsconfig.json`에 `./*` 기준으로 정의
- 현재 적용: PR 또는 로컬 검증에서 `npm run lint` 실행 가능
- 적용 후보: ESLint `no-restricted-imports`로 feature 간 직접 import와 deep import 금지
- 적용 후보: `eslint-plugin-boundaries` 또는 `dependency-cruiser`로 레이어 의존성 검증
